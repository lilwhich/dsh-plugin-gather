...
/** 注入临时分析 agent 的历史事件上限（超过取尾部，避免超大会话爆上下文）。 */
const HANDOFF_MAX_HISTORY_EVENTS = 6000;
		...
		ctx.webServer.register({
			...
			handler: async (req, res) => {
				try {
					...
					// 1) 历史：原顺序，裁剪到上限 + 尾部闭合 turn
					const history = balancedHistory(session.events, HANDOFF_MAX_HISTORY_EVENTS);

					// 2) 临时分析 agent：禁止一切工具；只读上下文并输出 Handoff
					...
					try {
						...
						const taskBoardText = body.taskBoard
							...
							: '';
						tempAgent.followup(createHandoffUserMessage(HANDOFF_ANALYZER_PROMPT + taskBoardText));
					...
					json(res, 200, { ok: true, newSessionId: newId });
				} catch (error) {
					json(res, 500, { ok: false, error: error instanceof Error ? error.message : String(error) });
				}
			}
