...
function gitRestore(cwd, id) {
	// Verify the id is one of OUR checkpoint commits before resetting.
...
function fsCheckpoint(cwd, title) {
	...
	if (last && last.kind === 'fs' && last.signature === fsSignature(cwd)) return null;
	...
	walk(cwd, '');
	return { id, title, time: nowLabel(), files, kind: 'fs', signature: fsSignature(cwd) };
}
...
function checkpointList(cwd) {
	...
	const mode = gitNow ? 'git' : index.mode;
	return {
		cwd,
		mode,
		gitRepo: gitNow,
