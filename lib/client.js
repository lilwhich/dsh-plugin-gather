window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// StatusBar 标签与左侧栏 ⋯ 菜单共用：读取当前模式 / 切换模式。
		...
		const StatusBar = function StatusBar(props) {
			...
			// Security Mode 标签（host 侧设置，见 /api/security/mode）
			useEffect(() => {
				let alive = true;
				async function pollMode() {
					try {
						...
						if (res.ok) {
							const json = await res.json();
							if (alive && json !== null && typeof json === 'object') setSecMode(json.mode ?? null);
						}
					} catch {
						/* keep the last mode */
					}
				...
				void pollMode();
				const timer = setInterval(() => void pollMode(), 30_000);
				return () => {
					alive = false;
					clearInterval(timer);
				};
			...
			// Security Mode 开关：标签即开关（点击切换，host 持久化）
			const secLabel = secModeLabel(secMode);
			if (secLabel !== null) {
				const toggleSec = () => {
					postSecurityMode(secModeNext(secMode)).then((m) => { if (m !== null) setSecMode(m); }).catch(() => { /* keep */ });
				};
				cells.push(h('button', {
					key: 'sec',
					style: { ...STAT_ITEM, color: 'var(--dsw-alias-state-success-primary)', cursor: 'pointer', background: 'transparent', border: 'none', padding: 0, font: 'inherit', lineHeight: 'inherit' },
					title: secMode === 'full-access-except-delete' ? '安全模式：删除文件时询问（点击切换为「完全访问」）' : '安全模式：完全访问（点击切换为「删除文件时询问」）',
					onClick: () => toggleSec()
				}, secLabel));
			}
			const row = [];
		...
		const LeftSidebarInner = function LeftSidebarInner(props) {
			...
			// Security Mode 开关状态（host 持久化，见 /api/security/mode）
			const [secMode, setSecMode] = useState(null);
			useEffect(() => {
				let alive = true;
				const sync = () => fetchSecurityMode().then((m) => { if (alive && m !== null) setSecMode(m); }).catch(() => { /* keep */ });
				void sync();
				...
				content = h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 } },
					...
					h('div', { style: { position: 'relative', flex: 'none', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', padding: '4px 6px', borderTop: '1px solid var(--dsw-alias-border-l2-darkmode-thin)' } },
						...
						menuOpen && h('div', { style: { position: 'absolute', right: '4px', bottom: 'calc(100% + 6px)', zIndex: 300, minWidth: '180px', background: 'var(--dsw-specific-input-major)', border: '1px solid var(--dsw-alias-border-l2-darkmode-thin)', borderRadius: '8px', boxShadow: '0 6px 24px rgba(0,0,0,0.25)', padding: '4px', display: 'flex', flexDirection: 'column' } },
							...
							h('div', { style: { height: '1px', margin: '4px 2px', background: 'var(--dsw-alias-border-l2-darkmode-thin)' } }),
							h('button', {
								style: { ...MENU_ITEM, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px' },
								title: '安全模式：删除文件时询问 ⇄ 完全访问（点击切换，即时生效并持久化）',
								onClick: () => toggleSecMode()
							},
								h('span', { style: { display: 'flex', alignItems: 'center', gap: '5px' } }, secModeLabel(secMode) ?? '🟢 安全模式'),
								h('span', {
									style: {
										width: '28px', height: '15px', borderRadius: '8px', flex: 'none', position: 'relative', display: 'inline-block',
										background: secMode === 'full-access' ? 'var(--dsw-alias-state-success-primary)' : 'var(--dsw-alias-interactive-bg-hover)',
										transition: 'background .15s'
									}
								}, h('span', {
									style: {
										position: 'absolute', top: '2px', left: secMode === 'full-access' ? '14px' : '2px',
										width: '11px', height: '11px', borderRadius: '50%', background: '#fff', transition: 'left .15s'
									}
								})))),
						gsHint && h('div', {
