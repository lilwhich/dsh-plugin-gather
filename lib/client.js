window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ══ Context-remaining indicator inside the composer ══════════════════
		...
		const CheckpointsTab = function CheckpointsTab(props) {
			...
			const refresh = useCallback(async () => {
				try {
					const res = await fetch('/my-better-dsh/api/checkpoints', { cache: 'no-store' });
			...
			async function openDetail(item) {
				...
				setConfirmStep(0);
				try {
					const res = await fetch(`/my-better-dsh/api/checkpoint/diff?id=${encodeURIComponent(item.id)}`, { cache: 'no-store' });
					const data = await res.json();
					setDetail(data);
				} catch (e) {
