			...
			async function doSave() {
				...
				try {
					...
					if (data.ok) {
						...
						setDirty(false);
						setSavedAt(Date.now());
						setNotice('已保存 ✓ 新会话立即生效；当前会话下一轮自动刷新');
						try {
			...
			try {
				content = h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 } },
					...
					mode === 'files'
						...
						: mode === 'settings'
							? h(GlobalSettingsTab, { compact: true })
					...
					h('div', { style: { position: 'relative', flex: 'none', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', padding: '4px 6px', borderTop: '1px solid var(--dsw-alias-border-l2-darkmode-thin)' } },
						...
						menuOpen && h('div', { style: { position: 'absolute', right: '4px', bottom: 'calc(100% + 6px)', zIndex: 300, minWidth: '132px', background: 'var(--dsw-specific-input-major)', border: '1px solid var(--dsw-alias-border-l2-darkmode-thin)', borderRadius: '8px', boxShadow: '0 6px 24px rgba(0,0,0,0.25)', padding: '4px', display: 'flex', flexDirection: 'column' } },
							h('button', { style: MENU_ITEM, onClick: () => { setMode('settings'); setMenuOpen(false); } }, '全局设定'),
						...
						h('button', {
							style: { position: 'relative', border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--dsw-alias-label-tertiary)', fontSize: '14px', width: '26px', height: '26px', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center' },
							title: '全局设定（对所有会话生效）',
							'aria-label': '全局设定',
							onClick: openGsMenu
						}, '⚙️', gsHint && h('span', {
