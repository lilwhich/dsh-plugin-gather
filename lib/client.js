window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// Polls /my-better-dsh/api/diff-review and renders the agent's file
		// changes as colored hunks, newest first, with a flash highlight on the
		// latest change and an auto-follow scroll (toggle 实时跟随).
		const DIFF_POLL_MS = 1500;
		const diffState = { lastSeq: 0 };
		const DIFF_STYLE = {
		...
		const ChangeCard = function ChangeCard(props) {
			...
			return h('div', { style: { border: '1px solid var(--dsw-alias-border-l2-darkmode-thin)', borderRadius: '8px', overflow: 'hidden', ...(flash ? { background: 'rgba(250,204,21,0.16)', boxShadow: '0 0 0 1px rgba(250,204,21,0.55)' } : {}) } },
				h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 8px', background: 'var(--dsw-alias-interactive-bg-hover)', fontSize: '12px' } },
		...
		const DiffReviewTab = function DiffReviewTab(props) {
			...
			// While the panel is visible, incoming changes are "read".
			useEffect(() => {
				if (props.visible) setUnread(0);
			}, [props.visible, entries]);
			...
			useEffect(() => {
				...
				async function poll(initial) {
					try {
						const after = initial ? 0 : diffState.lastSeq;
						const res = await fetch(`/my-better-dsh/api/diff-review?${cwdQuery}after=${after}`, { cache: 'no-store' });
						if (!res.ok) return;
						const data = await res.json();
						if (!data.ok || !alive) return;
						diffState.lastSeq = data.latestSeq || 0;
						if (Array.isArray(data.changes) && data.changes.length > 0) {
							const latest = data.changes[data.changes.length - 1];
							setEntries((prev) => {
								const known = new Set(prev.map((e) => e.seq));
								const fresh = data.changes.filter((e) => !known.has(e.seq));
								return [...fresh.reverse(), ...prev].slice(0, 60);
							});
							setFlashSeq(latest.seq);
							clearTimeout(flashTimer);
							flashTimer = window.setTimeout(() => setFlashSeq(null), 2500);
							if (props.visible) setUnread(0);
							else setUnread((u) => u + data.changes.length);
							if (follow) {
								window.setTimeout(() => {
									if (alive && listRef.current !== null) listRef.current.scrollTop = 0;
								}, 60);
							}
						}
					} catch {
				...
				void poll(true);
				const timer = setInterval(() => void poll(false), DIFF_POLL_MS);
				return () => {
					alive = false;
					clearInterval(timer);
					clearTimeout(flashTimer);
				};
			}, [cwdQuery, follow, props.visible]);
			...
			return h('div', { style: { padding: '10px 8px', display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px', color: 'var(--dsw-alias-label-primary)', minHeight: 0, flex: 1 } },
				...
				h('div', { ref: listRef, style: { flex: 1, minHeight: 0, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '6px' } },
					entries.map((e) => h(ChangeCard, { key: e.seq, entry: e, flash: e.seq === flashSeq }))));
		};
		...
		const LeftSidebarInner = function LeftSidebarInner(props) {
			...
			try {
				content = h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 } },
					...
					// ── bottom-right corner: settings entry (secondary menu) ──
					h('div', { style: { position: 'relative', flex: 'none', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', padding: '4px 6px', borderTop: '1px solid var(--dsw-alias-border-l2-darkmode-thin)' } },
						menuOpen && h('div', { style: { position: 'fixed', inset: 0, zIndex: 299 }, onClick: () => setMenuOpen(false) }),
						menuOpen && h('div', { style: { position: 'absolute', right: '4px', bottom: 'calc(100% + 6px)', zIndex: 300, minWidth: '132px', background: 'var(--dsw-specific-input-major)', border: '1px solid var(--dsw-alias-border-l2-darkmode-thin)', borderRadius: '8px', boxShadow: '0 6px 24px rgba(0,0,0,0.25)', padding: '4px', display: 'flex', flexDirection: 'column' } },
