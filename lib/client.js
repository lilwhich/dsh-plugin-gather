window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ══ i18n：跟随系统语言（zh → 中文，其它 → English）═══════════════════
		// 监听 languagechange 实时切换；组件用 useLocale() 取当前语言并强制重渲染。
		let zhLocale = true;
		try { zhLocale = (navigator.language || 'zh').toLowerCase().startsWith('zh'); } catch { zhLocale = true; }
		...
		const secModeLabel = (mode) => mode === 'full-access-except-delete' ? T('🟢 删除时确认', '🟢 Confirm deletes') : mode === 'full-access' ? T('🟢 完全访问', '🟢 Full access') : null;
		...
		const StatusBar = function StatusBar(props) {
			...
			const periodLabel = period === 'peak' ? T('高峰', 'Peak') : T('空闲', 'Off-peak');
			...
			const cells = [
				cell(`${T('余额', 'Balance')} ${balanceText}`, 'bal'),
				cell(`${T('本次已消耗', 'Spent')} ${spendText}`, 'spend'),
				cell(`${periodLabel} · ${T('距切换', 'switch in')} ${fmtCountdown(remain)}`, 'period', { color: periodColor }),
				cell(`${T('上下文', 'Context')} ${ctxText}`, 'ctx', { color: ctxColor })
			];
			...
			return h('div', {
				...
				style: DOCK_STYLE,
				title: T('余额/花费来自 DeepSeek 真实 API（60 秒刷新）；时段为官方峰谷定价（高峰 9-12、14-18 北京时间，空闲半价）；上下文为当前对话剩余窗口', 'Balance/spend from DeepSeek real API (60s refresh); periods follow official peak pricing (peak 9-12 & 14-18 Beijing time, off-peak half price); context = remaining window of the current conversation')
			},
				migratedFrom !== null && migratedFrom !== void 0 && h('span', {
					...
					title: T('此对话由 Context Handoff 创建', 'This conversation was created by a Context Handoff')
				}, T('🚚 已从上一对话迁徙', '🚚 Migrated from a previous conversation')),
				...row);
		};
		...
		const ContextRemain = function ContextRemain(props) {
			...
			const text = `${running ? `${T('回复中 · ', 'Replying · ')}` : ''}${T('剩', 'left')} ${fmtTokens(remain)}/${fmtTokens(window)}`;
			...
			return h('div', {
				...
				title: `${T('当前对话剩余上下文窗口', 'Remaining context of current conversation')} ${fmtTokens(remain)} / ${fmtTokens(window)}（${Math.round(ratio * 100)}%）`
			}, text);
		};
		...
		const CheckpointsTab = function CheckpointsTab(props) {
			...
			async function doRestore() {
				...
				try {
					...
					const data = await res.json();
					setMessage(data.ok ? T('已恢复 ✓', 'Restored ✓') : `${T('恢复失败：', 'Restore failed: ')}${data.error || T('未知错误', 'unknown error')}`);
					setConfirmStep(0);
					setDetail(null);
					setSelectedId(null);
					await refresh();
				} catch (e) {
					setMessage(`${T('恢复失败：', 'Restore failed: ')}${e instanceof Error ? e.message : String(e)}`);
				} finally {
					setRestoring(false);
				}
			}
			...
			const title = (items?.length ?? 0) > 0
				? 'CHECKPOINTS'
				: items === null ? 'CHECKPOINTS' : T('CHECKPOINTS（空）', 'CHECKPOINTS (empty)');
			...
			return h('div', { style: { padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px', color: 'var(--dsw-alias-label-primary)' } },
				h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' } },
					...
					h('span', { style: TAB.badge },
						mode === 'git' ? T('Git 快照', 'Git snapshots') : mode === 'fs' ? T('文件快照', 'File snapshots') : T('未就绪', 'Not ready'))
				),
				mode === 'fs' && h('div', { style: { ...TAB.meta, background: 'var(--dsw-alias-interactive-bg-hover)', borderRadius: '8px', padding: '6px 8px' } },
					T('非 Git 项目：使用文件复制快照（不初始化 Git）。恢复会覆盖快照中的文件。', 'Non-Git project: file-copy snapshots are used (Git is never initialized). Restore overwrites the snapshot files.')),
				error !== null && h('div', { style: { color: 'var(--dsw-alias-state-error-primary)', fontSize: '12px' } }, `${T('检查点接口不可用：', 'Checkpoint API unavailable: ')}${error}`),
				...
				selectedId !== null && h('div', { style: { display: 'flex', flexDirection: 'column', gap: '8px' } },
					...
					detail !== null && detail.ok && h('div', {
						style: TAB.pre
					}, detail.text || T('（无差异）', '(no diff)')),
					detail !== null && !detail.ok && h('div', { style: { color: 'var(--dsw-alias-state-error-primary)' } }, detail.error || T('加载失败', 'Load failed')),

					h('button', {
						style: { ...TAB.btn, ...TAB.btnDanger },
						onClick: () => setConfirmStep(1)
					}, T('恢复到此快照', 'Restore to this snapshot'))
				),
				...
				confirmStep > 0 && h('div', {
					...
					h('div', { style: { fontWeight: 600 } }, confirmStep === 1 ? T('恢复到此快照？', 'Restore to this snapshot?') : T('再次确认恢复', 'Confirm restore again')),
					h('div', { style: { fontSize: '12px', color: 'var(--dsw-alias-label-secondary)', lineHeight: '18px' } },
						confirmStep === 1
							? T('将把项目恢复到该检查点记录的状态。此操作不可撤销，请确认。', 'The project will be restored to the state recorded in this checkpoint. This cannot be undone — please confirm.')
							: T('再次确认：将丢弃该检查点之后的所有更改，并重置工作区。', 'Confirm again: all changes after this checkpoint will be discarded and the workspace reset.')),
					h('div', { style: { display: 'flex', justifyContent: 'flex-end', gap: '8px' } },
						h('button', { style: { ...TAB.btn, ...TAB.btnGhost }, onClick: () => setConfirmStep(0) }, T('取消', 'Cancel')),
						confirmStep === 1
							? h('button', { style: { ...TAB.btn, ...TAB.btnDanger }, onClick: () => setConfirmStep(2) }, T('下一步', 'Next'))
							: h('button', {
								style: { ...TAB.btn, ...TAB.btnDanger },
								disabled: restoring,
								onClick: () => void doRestore()
							}, restoring ? T('恢复中…', 'Restoring…') : T('确认恢复', 'Restore'))
					)
				))
			);
		};
		...
		const SecurityModeSwitch = function SecurityModeSwitch(props) {
			...
			return h('button', {
				...
				role: 'switch',
				'aria-checked': on,
				title: on ? T('安全模式：删除文件时询问（点击切换为「完全访问」）', 'Security: confirm before deleting files (click to switch to full access)') : T('安全模式：完全访问（点击切换为「删除文件时询问」）', 'Security: full access (click to switch to confirm-before-delete)'),
				onClick: () => toggle()
			},
				h('span', {
					style: {
				...
				h('span', { style: { fontSize: props.labelSize ?? '12px', color: 'var(--dsw-alias-label-primary)', whiteSpace: 'nowrap' } },
					props.short
						? (secMode === 'full-access-except-delete' ? T('删除时确认', 'Confirm deletes') : secMode === 'full-access' ? T('完全访问', 'Full access') : T('安全模式', 'Security'))
						: (secModeLabel(secMode) ?? T('安全模式', 'Security'))));
		};
		...
		const GlobalSettingsTab = function GlobalSettingsTab(props) {
			...
			const refresh = useCallback(async () => {
				try {
					...
					if (!res.ok) throw new Error(`HTTP ${res.status}`);
					const data = await res.json();
					if (!data.ok) throw new Error(data.error || T('读取失败', 'Read failed'));
					setInfo(data);
					setError(null);
			...
			async function doSave() {
				...
				try {
					...
					if (data.ok) {
						...
						setError(data.error || T('保存失败', 'Save failed'));
					}
				} catch (e) {
					setError(e instanceof Error ? e.message : String(e));
				} finally {
			...
			async function doReset() {
				...
				try {
					...
					if (data.ok) {
						...
						setDirty(false);
						setSavedAt(Date.now());
						setNotice(T('已恢复模板 ✓', 'Template restored ✓'));
						try {
							localStorage.removeItem(GS_DRAFT_KEY);
						} catch {
							/* ignore */
						...
						setError(data.error || T('恢复失败', 'Restore failed'));
					}
				} catch (e) {
					setError(e instanceof Error ? e.message : String(e));
				} finally {
			...
			return h('div', { style: { padding: compact ? '8px 6px' : '12px 10px', display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px', color: 'var(--dsw-alias-label-primary)', minHeight: 0, ...(compact ? { flex: 1 } : {}) } },
				h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' } },
					h('span', { style: { fontWeight: 600, fontSize: '13px', letterSpacing: '0.5px' } }, T('全局设定', 'Global Settings')),
					h('span', { style: { ...TAB.badge, ...(info !== null && info.active ? { background: 'var(--dsw-alias-state-success-primary)', color: '#fff' } : {}) } },
						info !== null ? (info.active ? (compact ? T('生效中', 'Active') : T('对所有会话生效', 'Applies to all sessions')) : T('未创建', 'Not created')) : T('读取中…', 'Loading…'))),
				h('div', { style: { ...TAB.meta, background: 'var(--dsw-alias-interactive-bg-hover)', borderRadius: '8px', padding: '6px 8px', lineHeight: '18px' } },
					T('编辑 ', 'Edit '), h('strong', null, info !== null ? info.display : '~/.dsh/AGENTS.md'),
					compact ? T('（所有会话生效；项目内 AGENTS.md / CLAUDE.md 优先级更高）', ' (applies to all sessions; project AGENTS.md / CLAUDE.md take precedence)') : T('（DSH 的用户级指令文件，模仿 CLAUDE.md）：所有会话启动时自动载入；项目目录下的 AGENTS.md / CLAUDE.md 优先级更高。', ' (DSH user-level instruction file, CLAUDE.md style): loaded automatically at every session start; project-level AGENTS.md / CLAUDE.md take precedence.')),
				error !== null && h('div', { style: { color: 'var(--dsw-alias-state-error-primary)', fontSize: '12px' } }, `${T('接口不可用：', 'API unavailable: ')}${error}`),
				...
				h('textarea', {
					...
					spellCheck: false,
					placeholder: T('在此编写对所有会话生效的设定（Markdown）…', 'Write settings that apply to every session here (Markdown)…'),
				...
				h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: 'var(--dsw-alias-label-tertiary)' } },
					...
					dirty && h('span', { style: { color: 'var(--dsw-alias-state-warning-primary)', fontWeight: 600 } }, T('未保存', 'Unsaved')),
					savedText !== null && h('span', null, `${T('上次保存', 'Saved at')} ${savedText}`),
					h('span', { style: { flex: 1 } }),
					!compact && h('button', { style: { ...TAB.btn, ...TAB.btnGhost }, onClick: copyPath }, T('复制路径', 'Copy path')),
					h('button', {
						style: { ...TAB.btn, ...TAB.btnGhost },
						onClick: () => setConfirmReset(true)
					}, T('恢复模板', 'Reset template')),
					h('button', {
				...
				confirmReset && h('div', {
					...
					h('div', { style: { fontWeight: 600 } }, T('恢复默认模板？', 'Reset to default template?')),
					h('div', { style: { fontSize: '12px', color: 'var(--dsw-alias-label-secondary)', lineHeight: '18px' } },
						T('将用内置示例模板覆盖 ~/.dsh/AGENTS.md 的全部内容，当前内容不可恢复。', 'The built-in sample template will overwrite all of ~/.dsh/AGENTS.md; the current content cannot be recovered.')),
					h('div', { style: { display: 'flex', justifyContent: 'flex-end', gap: '8px' } },
						h('button', { style: { ...TAB.btn, ...TAB.btnGhost }, onClick: () => setConfirmReset(false) }, T('取消', 'Cancel')),
						h('button', { style: { ...TAB.btn, ...TAB.btnDanger }, disabled: busy, onClick: () => void doReset() }, busy ? T('恢复中…', 'Restoring…') : T('确认恢复', 'Reset')))))
			);
		};
		...
		const ChangeCard = function ChangeCard(props) {
			...
			const toolLabel = entry.tool === 'write' ? T('写入', 'write')
				: entry.tool === 'edit' || entry.tool === 'str_replace_editor' ? T('编辑', 'edit')
				: entry.tool;
			return h('div', { style: { border: '1px solid var(--dsw-alias-border-l2-darkmode-thin)', borderRadius: '8px', overflow: 'hidden', ...(flash ? { background: 'rgba(250,204,21,0.16)', boxShadow: '0 0 0 1px rgba(250,204,21,0.55)' } : {}) } },
				h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 8px', background: 'var(--dsw-alias-interactive-bg-hover)', fontSize: '12px', cursor: 'pointer' }, onClick: () => setOpen((v) => !v), title: open ? T('收起', 'Collapse') : T('展开', 'Expand') },
					...
					h('span', { style: { flex: 1 } }),
					entry.newFile && h('span', { style: { ...TAB.badge, color: 'var(--dsw-alias-state-success-primary)' } }, T('新增', 'New')),
				...
				open && h('div', { style: { padding: '4px 0 6px' } },
					...
					entry.truncated && h('div', { style: { ...TAB.meta, padding: '2px 8px' } }, T('（diff 过长，已截断）', ' (diff too long, truncated)'))));
		};
				...
				return h('div', { style: { padding: '10px 8px', display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px', color: 'var(--dsw-alias-label-primary)', minHeight: 0, flex: 1 } },
					h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' } },
						...
						entries.length > 0 && h('span', { style: TAB.meta }, `${entries.length} ${T('条', 'items')}`),
						h('span', { style: { flex: 1 } }),
					...
					entries.length === 0 && h('div', { style: { color: 'var(--dsw-alias-label-tertiary)', textAlign: 'center', padding: '24px 8px', lineHeight: '20px', whiteSpace: 'pre-line' } },
						T('暂无改动\nAgent 修改文件后会实时显示在这里', 'No changes yet\nAgent file changes will appear here in real time')),
		...
		const FOOT_BTN = {
			...
			color: 'var(--dsw-alias-label-primary)', fontSize: '12px', lineHeight: '20px', padding: '2px 8px',
		...
		const ConfirmDialog = function ConfirmDialog(props) {
			return h('div', { style: CONFIRM_OVERLAY },
				h('div', { style: CONFIRM_BOX },
					h('div', { style: { fontWeight: 600 } }, props.title || T('确认', 'Confirm')),
					h('div', { style: { fontSize: '12px', color: 'var(--dsw-alias-label-secondary)', lineHeight: '18px', whiteSpace: 'pre-line' } }, props.text || ''),
					h('div', { style: { display: 'flex', justifyContent: 'flex-end', gap: '8px' } },
						h('button', {
						...
						h('button', {
							style: { ...MENU_ITEM, width: 'auto', color: '#fff', background: 'var(--dsw-alias-state-error-primary)', ...(props.confirmStyle ?? {}) },
							onClick: props.onConfirm
						}, props.confirmLabel || T('确认', 'Confirm')))));
		};
		...
		const FileTree = function FileTree(props) {
			...
			async function doDelete() {
				...
				try {
					...
					if (data.ok) {
						...
						setError(data.error || T('删除失败', 'Delete failed'));
					}
				} catch (e) {
					setError(e instanceof Error ? e.message : String(e));
				} finally {
				...
				for (const f of files) {
					...
					parts.push(h('div', { key: `f:${rel}`, style: { position: 'relative' } },
						...
						menuFor === rel && h(MoreMenu, {
							...
							items: [
								{ label: isPinned(rel) ? T('取消置顶', 'Unpin') : T('置顶', 'Pin'), onClick: () => onTogglePin(rel) },
								{ label: T('复制路径', 'Copy path'), onClick: () => copyPath(rel) },
								{ label: T('删除', 'Delete'), danger: true, onClick: () => setConfirmDelete({ rel, isDir: false }) }
							]
						})));
				}
				return parts;
					...
					h('div', { style: { position: 'relative' } },
						...
						menuFor === rel && h(MoreMenu, {
							...
							items: [
								{ label: isPinned(rel) ? T('取消置顶', 'Unpin') : T('置顶', 'Pin'), onClick: () => onTogglePin(rel) },
								{ label: T('复制路径', 'Copy path'), onClick: () => copyPath(rel) },
								{ label: T('删除', 'Delete'), danger: true, onClick: () => setConfirmDelete({ rel, isDir: true }) }
							]
						})),
			...
			return h('div', { style: { flex: 1, minHeight: 0, overflowY: 'auto', padding: '4px 2px', fontSize: '13px' } },
				...
				root === void 0 && error === null && h('div', { style: { color: 'var(--dsw-alias-label-tertiary)', padding: '12px', textAlign: 'center' } }, T('加载中…', 'Loading…')),
				confirmDelete !== null && h(ConfirmDialog, {
					title: confirmDelete.isDir ? T('删除目录？', 'Delete directory?') : T('删除文件？', 'Delete file?'),
					text: `${T('将永久删除', 'Permanently delete')}「${confirmDelete.rel}」，${T('此操作不可撤销。', 'this cannot be undone.')}`,
					confirmLabel: busy ? T('删除中…', 'Deleting…') : T('确认删除', 'Delete'),
		...
		const SessionList = function SessionList(props) {
			...
			return h('div', { style: { flex: 1, minHeight: 0, overflowY: 'auto', padding: '4px 2px', fontSize: '13px' } },
				sorted.length === 0 && h('div', { style: { color: 'var(--dsw-alias-label-tertiary)', textAlign: 'center', padding: '16px 0' } }, T('暂无会话', 'No sessions')),
				sorted.map((s) => {
					...
					return h('div', { key: s.id, style: { position: 'relative' } },
						...
						menuFor === s.id && h(MoreMenu, {
							onClose: () => setMenuFor(null),
							items: [
								{ label: pinned ? T('取消置顶', 'Unpin') : T('置顶', 'Pin'), onClick: () => onTogglePin(s.id) },
								{ label: T('删除', 'Delete'), danger: true, onClick: () => setConfirmDelete(s.id) }
							]
						}));
				}),
				confirmDelete !== null && h(ConfirmDialog, {
					title: T('删除会话？', 'Delete session?'),
					text: T('将归档该会话（从列表中隐藏，不删除对话记录）。', 'The session will be archived (hidden from the list; the conversation history is kept).'),
					confirmLabel: busy ? T('删除中…', 'Deleting…') : T('确认删除', 'Delete'),
					onCancel: () => setConfirmDelete(null),
					onConfirm: () => {
						setBusy(true);
		...
		const LeftSidebarInner = function LeftSidebarInner(props) {
			...
			// ══ 对话迁徙（🚚）：二级确认 → 分析当前对话 → Context Handoff → 新 Conversation ══
			// 只复用 DSH 自身 Agent/LLM Runtime（host /api/handoff），无外部 API。
			const HANDOFF_STAGE_TEXT = {
				preprocessing: T('正在预处理当前对话……', 'Preprocessing conversation…'),
				analyzing: T('正在分析当前对话……', 'Analyzing conversation…'),
				extracting: T('正在提取任务状态……', 'Extracting task state…'),
				generating: T('正在生成迁徙上下文……', 'Generating handoff context…')
			};
			const [handoffOpen, setHandoffOpen] = useState(false);      // 二级确认弹框
			const [handoffState, setHandoffState] = useState(null);     // {busy,stage,done,tokens,error}
			useEffect(() => {
				if (handoffState === null || handoffState.busy !== true) return;
				const stages = ['preprocessing', 'analyzing', 'extracting', 'generating'];
				const timer = setInterval(() => {
			...
			const runHandoff = () => {
				const sessionId = sessions !== null && sessions !== void 0 ? sessions.current : null;
				if (!sessionId) { setHandoffState({ busy: false, error: T('无法确定当前会话', 'Cannot determine the current session') }); return; }
				if (handoffState !== null && handoffState.busy) return;
				setHandoffOpen(false);
				setHandoffState({ busy: true, stage: 'preprocessing' });
				let taskBoard = null;
				...
				fetch('/my-better-dsh/api/handoff', {
					...
					if (data !== null && typeof data === 'object' && data.ok === true && data.newSessionId) {
						const tokens = typeof data.estimatedTokens === 'number' ? data.estimatedTokens : null;
						setHandoffState({ busy: false, done: true, tokens });
						openSession(data.newSessionId);
						setTimeout(() => setHandoffState((prev) => (prev !== null && prev.done ? null : prev)), 6000);
					} else {
						setHandoffState({ busy: false, error: (data && data.error) || T('未知错误', 'Unknown error') });
					}
				}).catch((e) => {
			...
			const hsStageText = hsBusy ? (HANDOFF_STAGE_TEXT[handoffState.stage] || T('正在分析当前对话……', 'Analyzing conversation…')) : null;
			const hsDoneText = handoffState !== null && handoffState.done === true
				? (typeof handoffState.tokens === 'number' ? `${T('✅ 迁徙完成 · 约', '✅ Handoff done · ~')} ${fmtTokens(handoffState.tokens)} tokens` : T('✅ 迁徙完成', '✅ Handoff complete'))
				: null;
			const hsErrorText = handoffState !== null && handoffState.error ? T('❌ 迁徙失败，原对话未受影响。', '❌ Handoff failed; the original conversation is unaffected.') : null;
			const savePins = (next) => {
				setPins(next);
				try {
			...
			try {
				content = h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 } },
					h('div', { style: { display: 'flex', gap: '4px', padding: '8px 4px', borderBottom: '1px solid var(--dsw-alias-border-l2-darkmode-thin)', flex: 'none' } },
						tab('files', T('文件', 'Files'), '📁'),
						tab('sessions', T('会话', 'Sessions'), '💬'),
						tab('outline', T('大纲', 'Outline'), '🗂')),
					mode === 'files'
						? h(FileTree, {
					...
					// ── bottom bar: security-mode switch · handoff · settings（均分等大）──
					h('div', { style: { position: 'relative', flex: 'none', display: 'flex', justifyContent: 'flex-end', alignItems: 'stretch', gap: '5px', padding: '4px 6px', borderTop: '1px solid var(--dsw-alias-border-l2-darkmode-thin)' } },
						...
						menuOpen && h('div', { style: { position: 'absolute', right: '4px', bottom: 'calc(100% + 6px)', zIndex: 300, minWidth: '180px', background: 'var(--dsw-specific-input-major)', border: '1px solid var(--dsw-alias-border-l2-darkmode-thin)', borderRadius: '8px', boxShadow: '0 6px 24px rgba(0,0,0,0.25)', padding: '4px', display: 'flex', flexDirection: 'column' } },
							h('button', { style: MENU_ITEM, onClick: () => { lastModeRef.current = mode; setMode('settings'); setMenuOpen(false); } }, T('全局设定', 'Global Settings')),
							h('div', { style: { ...TAB.meta, padding: '2px 10px' } }, T('对所有会话生效的指令', 'Instructions that apply to every session'))),
						gsHint && h('div', {
						...
						}, T('⚙️ 全局（所有会话生效）', '⚙️ Global (applies to all sessions)')),
						// 删除时确认开关（与迁徙/全局同等大小）
						h(SecurityModeSwitch, {
							short: true,
							style: { ...FOOT_BTN, flex: 1, justifyContent: 'center' }
						}),
						// 对话迁徙（二级确认）
						h('button', {
							style: { ...FOOT_BTN, flex: 1, justifyContent: 'center', ...(hsBusy ? { cursor: 'wait', opacity: 0.75 } : {}) },
							title: T('分析当前对话并生成 Context Handoff，创建新对话继续工作（原对话保持不变）', 'Analyze the current conversation into a Context Handoff and create a new conversation to continue (the original conversation stays unchanged)'),
							disabled: hsBusy,
							onClick: () => setHandoffOpen(true)
						}, hsBusy ? hsStageText : T('🚚 对话迁徙', '🚚 Handoff')),
						// 全局设定（齿轮图标 + 「全局」）
						h('button', {
							style: { ...FOOT_BTN, flex: 1, justifyContent: 'center', position: 'relative' },
							title: T('全局设定（对所有会话生效）', 'Global settings (applies to all sessions)'),
							'aria-label': T('全局设定', 'Global Settings'),
							onClick: openGsMenu
						}, '⚙️', h('span', null, T('全局', 'Global')), gsHint && h('span', {
						...
						// 迁徙状态提示（done/error，显示在底部栏上方）
						(hsDoneText !== null || hsErrorText !== null) && h('div', {
							style: { position: 'absolute', right: '6px', bottom: 'calc(100% + 6px)', zIndex: 302, maxWidth: '90%', background: 'var(--dsw-specific-input-major)', border: '1px solid var(--dsw-alias-border-l2-darkmode-thin)', borderRadius: '8px', padding: '6px 10px', fontSize: '12px', boxShadow: '0 6px 24px rgba(0,0,0,0.25)', wordBreak: 'break-all', color: hsErrorText !== null ? 'var(--dsw-alias-state-error-primary)' : 'var(--dsw-alias-state-success-primary)' }
						}, hsErrorText !== null ? hsErrorText : hsDoneText,
							hsErrorText !== null && h('button', { style: { ...FOOT_BTN, marginLeft: '8px', color: 'var(--dsw-alias-label-primary)' }, onClick: () => void runHandoff() }, T('重试', 'Retry'))),
						// 二级确认弹框
						handoffOpen && h(ConfirmDialog, {
							title: T('🚚 对话迁徙', '🚚 Conversation Handoff'),
							text: T('将分析当前对话并生成 Context Handoff，随后创建新对话继续工作（新对话只携带 Handoff，不包含完整历史）。\n\n原对话保持不变。', 'The current conversation will be analyzed into a Context Handoff, then a new conversation will be created to continue (the new conversation only carries the Handoff, not the full history).\n\nThe original conversation stays unchanged.'),
							confirmLabel: T('确认迁徙', 'Migrate'),
							confirmStyle: { background: 'var(--dsw-alias-state-info-primary, #3b82f6)', color: '#fff' },
							onCancel: () => setHandoffOpen(false),
							onConfirm: () => void runHandoff()
						})),
		...
		const ConversationOutline = function ConversationOutline(props) {
			...
			return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 } },
				h('div', { style: { display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 4px 4px', flex: 'none' } },
					h('span', { style: { fontWeight: 600, fontSize: '13px', letterSpacing: '0.5px' } }, T('🗂 对话大纲', '🗂 Conversation Outline')),
					h('span', { style: { flex: 1 } }),
					items.length > 0 && h('span', { style: { ...TAB.meta } }, `${items.length} ${T('条', 'items')}`)),
				h('div', { style: { padding: '0 6px 4px', flex: 'none' } },
					h('input', { style: OUTLINE_STYLE.search, placeholder: T('🔍 搜索对话…', '🔍 Search conversation…'), value: query, onChange: (e) => setQuery(e.target.value) })),
					...
					h('div', {
						...
						filtered.length === 0
							? h('div', { style: { color: 'var(--dsw-alias-label-tertiary)', textAlign: 'center', padding: '20px 8px', lineHeight: '20px', fontSize: '12px' } },
								q !== '' ? T('无匹配节点', 'No matching items') : T('暂无用户消息', 'No user messages'))
							: filtered.map((it) => {
		...
		const DeleteConfirmOverlay = function DeleteConfirmOverlay(props) {
			...
			const title = isDir ? T('⚠️ 确认删除目录', '⚠️ Confirm directory delete') : isBatch ? T('⚠️ 确认批量删除', '⚠️ Confirm bulk delete') : T('⚠️ 确认删除', '⚠️ Confirm delete');
			const body = [];
			if (isDir) {
				body.push(h('div', null, T('Agent 正在尝试删除目录：', 'The agent is trying to delete a directory:')));
				body.push(h('div', { style: DELETE_DIALOG_STYLE.path }, `📁 ${paths[0] ?? ''}`));
				const st = pending.dirStats;
				if (st !== null && st !== void 0 && typeof st === 'object') {
					body.push(h('div', null, `${T('包含：', 'Contains: ')}${st.files} ${T('个文件、', 'files, ')}${st.dirs} ${T('个子目录', 'subdirectories')}${st.truncated === true ? T('（统计已截断）', ' (stats truncated)') : ''}`));
				} else {
					body.push(h('div', null, T('包含内容无法统计（可能不存在或无权限）。', 'Contents cannot be counted (may not exist or no permission).')));
				}
			} else if (isBatch) {
				body.push(h('div', null, `${T('Agent 正在尝试删除', 'The agent is trying to delete')} ${paths.length} ${T('个文件：', 'files:')}`));
				const shown = paths.slice(0, 8);
				for (const p of shown) body.push(h('div', { style: DELETE_DIALOG_STYLE.path }, `📄 ${p}`));
				if (paths.length > shown.length) body.push(h('div', null, `${T('… 等共', '… and')} ${paths.length} ${T('个', 'more in total')}`));
			} else {
				body.push(h('div', null, T('Agent 正在尝试删除：', 'The agent is trying to delete:')));
				body.push(h('div', { style: DELETE_DIALOG_STYLE.path }, `📄 ${paths[0] ?? ''}`));
				body.push(h('div', null, T('此操作将删除文件。', 'This will delete the file.')));
			}
			if (pending.recursive === true && !isDir) body.push(h('div', null, T('（递归删除）', ' (recursive)')));
			...
			return h('div', { style: DELETE_DIALOG_STYLE.wrap },
				h('div', { style: DELETE_DIALOG_STYLE.box },
					...
					pending.hasCheckpoint === true && h('div', { style: DELETE_DIALOG_STYLE.hint }, T('🛡 删除前已有 Checkpoint，可在需要时恢复。', '🛡 A checkpoint exists from before the delete; you can restore if needed.')),
					h('div', { style: DELETE_DIALOG_STYLE.btnRow },
						h('button', { style: DELETE_DIALOG_STYLE.btnCancel, disabled: busy, onClick: () => resolve(false) }, T('取消', 'Cancel')),
						h('button', { style: DELETE_DIALOG_STYLE.btnAllow, disabled: busy, onClick: () => resolve(true) }, busy ? T('处理中…', 'Working…') : T('允许删除', 'Allow delete')))));
		};
