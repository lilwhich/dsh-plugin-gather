window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const SecurityModeSwitch = function SecurityModeSwitch(props) {
			...
			return h('button', {
				...
				h('span', { style: { fontSize: props.labelSize ?? '12px', color: 'var(--dsw-alias-label-primary)', whiteSpace: 'nowrap' } },
					props.short
						? (secMode === 'full-access-except-delete' ? '删除时确认' : secMode === 'full-access' ? '完全访问' : '安全模式')
		...
		const LeftSidebarInner = function LeftSidebarInner(props) {
			...
			const hsStageText = hsBusy ? (HANDOFF_STAGE_TEXT[handoffState.stage] || '正在分析当前对话……') : null;
			const hsDoneText = handoffState !== null && handoffState.done === true
				? (typeof handoffState.tokens === 'number' ? `✅ 迁徙完成 · 约 ${fmtTokens(handoffState.tokens)} tokens` : '✅ 迁徙完成')
				: null;
			const hsErrorText = handoffState !== null && handoffState.error ? '❌ 迁徙失败，原对话未受影响。' : null;
			const savePins = (next) => {
				setPins(next);
				try {
				...
				content = h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 } },
					...
					h('div', { style: { position: 'relative', flex: 'none', display: 'flex', justifyContent: 'flex-end', alignItems: 'stretch', gap: '5px', padding: '4px 6px', borderTop: '1px solid var(--dsw-alias-border-l2-darkmode-thin)' } },
						...
						h('button', {
							...
							'aria-label': '全局设定',
							onClick: openGsMenu
						}, '⚙️', h('span', null, '全局'), gsHint && h('span', {
						...
						(hsDoneText !== null || hsErrorText !== null) && h('div', {
							...
							hsErrorText !== null && h('button', { style: { ...FOOT_BTN, marginLeft: '8px', color: 'var(--dsw-alias-label-primary)' }, onClick: () => void runHandoff() }, '重试')),
						// 二级确认弹框
						handoffOpen && h(ConfirmDialog, {
							title: '🚚 对话迁徙',
							text: '将分析当前对话并生成 Context Handoff，随后创建新对话继续工作（新对话只携带 Handoff，不包含完整历史）。\n\n原对话保持不变。',
							confirmLabel: '确认迁徙',
