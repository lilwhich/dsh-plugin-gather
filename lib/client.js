window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const StatusBar = function StatusBar(props) {
			...
			// Security Mode 显示标签（切换入口在全局设定页 / 左侧 ⋯ 菜单，见 SecurityModeSwitch）
		...
		const GlobalSettingsTab = function GlobalSettingsTab(props) {
			...
			return h('div', { style: { padding: compact ? '8px 6px' : '12px 10px', display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '13px', color: 'var(--dsw-alias-label-primary)', minHeight: 0, ...(compact ? { flex: 1 } : {}) } },
				...
				// ── 安全模式（独立开关）：删除文件/目录时询问，其余操作完全访问 ──
				h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', background: 'var(--dsw-alias-interactive-bg-hover)', borderRadius: '8px', padding: '8px 10px' } },
					h('div', { style: { display: 'flex', flexDirection: 'column', gap: '2px', minWidth: 0 } },
						h('span', { style: { fontWeight: 600, fontSize: '12px' } }, '安全模式'),
						h('span', { style: { ...TAB.meta, fontSize: '11px', lineHeight: '16px' } }, '删除文件/目录时询问，其余操作保持完全访问')),
		...
		const LeftSidebarInner = function LeftSidebarInner(props) {
			...
			// Security Mode 开关状态（订阅共享源，任一入口切换即时同步）
			const [secMode, setSecMode] = useState(sharedSecMode);
			useEffect(() => {
				const unsub = subscribeSecMode(setSecMode);
				let alive = true;
			...
			const toggleSecMode = () => {
				postSecurityMode(secModeNext(secMode)).catch(() => { /* keep */ });
			};
