window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const StatusBar = function StatusBar(props) {
			...
			const runHandoff = () => {
				...
				fetch('/my-better-dsh/api/handoff', {
					...
					if (data !== null && typeof data === 'object' && data.ok === true && data.newSessionId) {
						// host 本地预处理后返回实际分析量估算（约 XX tokens）
			...
			const hsDoneTokens = handoffState !== null && handoffState.done === true && typeof handoffState.tokens === 'number' ? handoffState.tokens : null;
			const hsText = handoffState === null ? null
				: hsBusy ? (HANDOFF_STAGE_TEXT[handoffState.stage] || '正在分析当前对话……')
				: handoffState.done ? (hsDoneTokens !== null ? `✅ 迁徙完成 · 分析约 ${fmtTokens(hsDoneTokens)} tokens` : '✅ 迁徙完成')
