window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const StatusBar = function StatusBar(props) {
			...
			// Security Mode 标签：🟢 完全访问（删除文件时询问）
			if (secMode === 'full-access-except-delete') {
				cells.push(cell('🟢 删除时确认', 'sec', { color: 'var(--dsw-alias-state-success-primary)', cursor: 'default' }));
			} else if (secMode === 'full-access') {
				cells.push(cell('🟢 完全访问', 'sec', { color: 'var(--dsw-alias-state-success-primary)', cursor: 'default' }));
			}
			const row = [];
			cells.forEach((c, i) => {
