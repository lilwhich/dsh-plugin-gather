window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const FileTree = function FileTree(props) {
			...
			// Tree base = the CURRENT session's workspace (not the server cwd).
			const cwd = props.cwd || '';
			...
			function renderChildren(node, parentRel, depth) {
				...
				for (const f of node.files) {
					...
					parts.push(h('button', {
						key: `f:${rel}`,
						style: LEFT_ROW(depth),
						title: rel,
						onClick: () => props.onOpenFile(rel)
					}, h('span', { style: { flex: 'none' } }, '📄'), h('span', { style: { overflow: 'hidden', textOverflow: 'ellipsis' } }, f.name)));
				}
				return parts;
			}
		...
		const SessionList = function SessionList(props) {
			...
			const rows = (sessions?.ids ?? []).map((id) => sessions.byId[id]).filter((s) => s !== void 0 && !s.blank);
			return h('div', { style: { flex: 1, minHeight: 0, overflowY: 'auto', padding: '4px 2px', fontSize: '13px' } },
				rows.length === 0 && h('div', { style: { color: 'var(--dsw-alias-label-tertiary)', textAlign: 'center', padding: '16px 0' } }, '暂无会话'),
				rows.map((s) => {
		...
		const LeftSidebar = function LeftSidebar(props) {
			...
			return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 } },
				...
				mode === 'files'
					? h(FileTree, {
						cwd: sessions?.current ? sessions.byId[sessions.current]?.cwd : null,
						onOpenFile: (rel) => openFile(sessions?.current, rel)
					})
					: h(SessionList, { sessions, openSession }));
		};

		const inject = ['slots', 'betterSidebar', 'sessions'];
		...
		function apply(ctx) {
			...
			// renders). DIRECT register (not the deferred slots.inject): the graph
			// edge guarantees ui-sidebar declared the slot before our apply, so
			// any error is caught here and logged with our marker — never silently
			// swallowed by the deferred path.
			try {
				console.log('[my-better-dsh] registering left sidebar into sidebar.workspaces...');
				ctx.slots.register({
				...
				console.log('[my-better-dsh] left sidebar registered OK');
			} catch (error) {
				console.error('[my-better-dsh] left sidebar registration failed:', error);
			}
		}
