window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const LeftSidebar = function LeftSidebar(props) {
			const { useSessions, openSession, openFile } = props;
		...
		function apply(ctx) {
			...
			// Files/Sessions two-mode view (priority -100 < default 0 → lowest renders).
			// Defensive try/catch: a registration failure must never take down the
			// rest of the plugin (status bar / checkpoints).
			try {
