...
| 能力 | 原生 dsh web | dsh-web-ui 全家桶（12 项） | **my_better-dsh（本包）** |
|------|:------------:|:--------------------------:|:-------------------------:|
| **🆕 本包独有（8 大功能）** | | | |
| VSCode 左侧栏（文件 / 会话 / 大纲） | 无 | 无 | ✅ 内置 |
| 账户状态栏（余额 · 消耗 · 峰谷 · 上下文） | 无 | 仅实时吞吐 | ✅ 内置（更深） |
| 输入框内上下文指示 | 无 | 无 | ✅ 内置 |
| 🛡️ 安全模式（删除确认拦截） | 无 | 无 | ✅ 内置 |
| 🚚 对话迁徙（Context Handoff，99% token 压缩） | 无 | 无 | ✅ 内置 |
| 📸 Checkpoint 快照（Git / 文件双轨） | 无 | 无 | ✅ 内置 |
| 👁️ Diff Review（实时行级 LCS diff） | 无 | 无 | ✅ 内置 |
| ⚙️ 全局设定（编辑 `~/.dsh/AGENTS.md`） | 无 | 无 | ✅ 内置 |
| **📦 全家桶精选 5 项 + @ 引用** | | | |
| 右侧面板（预览 / 文件树 / SCM） | 无 | ✅ | ✅ 精选 |
| 任务看板（多列 + cron 定时） | 无 | ✅ | ✅ 精选 |
| 实时吞吐（Token 用量 / 统计） | 无 | ✅ | ✅ 精选 |
| 全家桶设置页 | 无 | ✅ | ✅ 精选 |
| 皮肤中心（10 款主题） | 无 | ✅ | ✅ 精选 |
| `@` 引用文件（dsh-at-file） | 无 | 无 | ✅ 另含 |
| **🚫 全家桶有但本包不装（保持小白友好）** | | | |
| 梁神模式预设（dsh-liangshen） | 无 | ✅ | ❌ 不装 |
| 移动端远程（dsh-remote-web-ui） | 无 | ✅ | ❌ 不装 |
| Git 图谱（dsh-git-graph） | 无 | ✅ | ❌ 不装 |
| 图像理解（dsh-tool-describe-image） | 无 | ✅ | ❌ 不装 |
| SSH 远程运维（dsh-ssh） | 无 | ✅ | ❌ 不装 |
| 桌面宠物（dsh-pet） | 无 | ✅ | ❌ 不装 |

> 一句话：**本包 = 全家桶的精选 5 项 + 本包独有的 8 大功能 + `@` 引用文件**，不装花哨/偏门项。
